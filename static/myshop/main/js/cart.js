$(function () {
    $(".confirm").click(function () {
        ...
        $.getJSON("/myshop/changecartstate/", {'cartid': cartid}, function (data) {
            console.log(data);
            if (data['status'] === 200) {
    ...
    $(".subShopping").click(function () {
        ...
        var cartid = $li.attr("cartid");
        $.getJSON("/myshop/subshopping/", {"cartid": cartid}, function (data) {
            console.log(data);
            if (data['status'] === 200) {
                $("#total_price").html(data['total_price']);
    ...
    $(".all_select").click(function () {
        ...
        if (unselect_list.length > 0) {
            $.getJSON("/myshop/allselect/", {"cart_list": unselect_list.join("#")}, function (data) {
                console.log(data);
                if (data['status'] === 200) {
                    $(".confirm").find("span").find("span").html("√");
            ...
            if (select_list.length > 0) {
                $.getJSON("/myshop/allselect/", {"cart_list": select_list.join("#")}, function (data) {
                    console.log(data);
                    if (data['status'] === 200) {
                        $(".confirm").find("span").find("span").html("");
                        $all_select.find("span").find("span").html("");
    ...
    $("#make_order").click(function () {
        ...
        $.getJSON("/myshop/makeorder/", function (data) {
            console.log(data);
            if (data['status'] === 200) {
                window.open('/myshop/orderdetail/?orderid=' + data['order_id'], target = "_self");
            }
        })
    })
